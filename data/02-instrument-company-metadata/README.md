...
`isin_not_found` (866 — ISIN not found by Yahoo search; typically delisted,
merged, or renamed since Nov 2021) or `no_isin` (11 — no ISIN available in the
tick files and no direct ticker match).

## 2026-07 re-run over the extended universe

With the weekday files `day-08`/`day-09` downloaded (see dataset 01), the
universe was re-extracted with `extract_symbol_universe.py` into the canonical
`symbols_week.txt` (5,502 symbols: the three additions are weekday-only Paris
index feeds `I2HCA.FR`, `I2WME.FR`, `I2WVE.FR`) and `sym_isin_week.txt`
(5,490 mappings; notably `A3E5E4.ETR` gained a real ISIN).

`resolve_yahoo_metadata.py` then re-ran the two-pass resolution for all 877
unresolved equities (results cached in `cache_yahoo_resolution.jsonl`,
gitignored): **zero** could be resolved — confirming the unresolved set
reflects genuine identifier drift (securities delisted/merged/renamed between
Nov 2021 and today), not missing effort. `table1`–`table3` are regenerated by
the script (content unchanged for `table1`, canonical ASCII ordering).
