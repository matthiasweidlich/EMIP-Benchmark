...
| News and event context | Text/event signals around companies and sectors | GDELT-style news events | Current repo: one week of historic GDELT data for Nov 8-14, 2021, with 430,918 GKG records scanned, 49,298 energy-relevant records, and 224,424 document-event links in compressed enrichment files. | Loaded as a static historic corpus for baseline analysis and also replayed by publication time as a low-throughput contextual stream. |
...
GDELT has two roles in the default benchmark. The full one-week slice is available before the run as historic news context for baselines, trend analysis, and anomaly detection. The same records are also replayed by publication time during the run as a low-throughput stream, allowing systems to test joins between fresh market ticks and slower news/event signals.
...
| News and event context | Low-throughput stream: the one-week GDELT slice has 49,298 energy-relevant GKG records over seven days, about 0.08 records/s on average; 224,424 document-event links, about 0.37 links/s on average. | The same one-week slice is also loaded as historic context before the run. Real publication traffic is bursty, so streaming replay should use publication time rather than a uniform rate. |
